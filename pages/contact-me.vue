		...
		<div id="page-menu" class="w-full h-full flex flex-col border-right">
			<div id="contacts" class="submenu">
				<div class="title" @click="open('contacts')">
					...
					<h3>{{ t('contacts.direct.title') }}</h3>
				</div>
				<div id="links">
					<div v-for="(source, key) in directContactSources" :key="key" class="link">
			...
			<div id="find-me-in" class="submenu border-top">
				<div class="title" @click="open('find-me-in')">
					<img class="arrow" src="/icons/arrow.svg" alt="" />
					<h3>{{ t('contacts.find_me_also_in.title') }}</h3>
				</div>
				<div id="links">
					<div v-for="(source, key) in findMeAlsoInSources" :key="key" class="link">
...
const findMeAlsoInSources = computed(() => {
	return messages[locale.value].contacts.find_me_also_in.sources
})

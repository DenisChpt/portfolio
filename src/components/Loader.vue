...
gsap.defaults({
  ...
  smoothOrigin: true  // Rendre plus fluides les animations avec changement d'origine
});
...
function initProgressCircle() {
  ...
  const staticValue = `${staticDash.toFixed(4)} ${staticGap.toFixed(4)} ${staticDash.toFixed(4)} ${staticGap.toFixed(4)}`
...
function setProgressCircleProperties(e: number = 100) {
  ...
  const staticValue = `${staticDash.toFixed(4)} ${staticGap.toFixed(4)} ${staticDash.toFixed(4)} ${staticGap.toFixed(4)}`
  const hoverValue = `${hoverDash.toFixed(4)} ${hoverGap.toFixed(4)} ${hoverDash.toFixed(4)} ${hoverGap.toFixed(4)}`
...
// Nouvelle fonction pour l'animation de fond optimisée
function initBackgroundAnimation() {
  ...
  // Ajuster la taille du canvas pour éviter le pixelisation
    ...
    onComplete() {
      ...
      gsap.to(progressTextInner.value!, {
        ...
        onComplete: () => {
          ...
          if (!progressCircle.value) return
          const finalValue = progressCircle.value.style.getPropertyValue("--circle-dash-array-static")
          const finalOffset = progressCircle.value.style.getPropertyValue("--circle-dash-offset-static")
          ...
          gsap.to(progressCircle.value, {
            ...
            onComplete: () => {
              ...
              gsap.fromTo('.preloader-cta-text-inner', {
                opacity: 0,
                y: 20
              }, {
                opacity: 1,
...
function handleCircleHover(isEntering: boolean) {
  ...
  if (isEntering) {
    ...
    const hoverDashArray = circle.style.getPropertyValue("--circle-dash-array-hover") || '325.27 36.09 325.27 36.09'
    ...
    // Animation du texte
    tl.to('.preloader-cta-text-static', {
      y: '-100%',
      opacity: 0,
      duration: 0.5,
      ease: "power2.out",
    ...
    tl.to('.preloader-cta-text-hover', {
      y: 0,
      opacity: 1,
      duration: 0.5,
      ease: "power2.out",
    ...
    const staticDashArray = circle.style.getPropertyValue("--circle-dash-array-static") || '339.29 37.69 339.29 37.69'
    const staticDashOffset = circle.style.getPropertyValue("--circle-dash-offset-static") || '339.29'
...
onMounted(() => {
  ...
  initBackgroundAnimation() // Initialise l'animation de fond
  animateIn()
  simulateProgress()
})
...
<template>
  <div v-if="loading" class="loader-container fixed inset-0 flex items-center justify-center bg-[#090A0F]">
...
@media (min-width: 1000px) {
  .loader-container {
    font-size: clamp(14px, .92593vw, 18px);
  }
}
</style>
